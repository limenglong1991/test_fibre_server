...
Encoder::Encoder(Config_t& config) :config_(config)
{
    update_pll_gains();
...
//        if (!axis_->motor_.enqueue_voltage_timings(0.0f, voltage_magnitude))
...
//        return false;
    ...
    axis_->run_control_loop([&](){
        ...
        //float v_alpha = -voltage_magnitude * our_arm_sin_f32(phase);
        //float v_beta = voltage_magnitude * our_arm_cos_f32(phase);
        if (!axis_->motor_.enqueue_voltage_timings(v_alpha, v_beta, phase_))
            return false; // error set inside enqueue_voltage_timings
        axis_->motor_.log_timing(Motor::TIMING_LOG_ENC_CALIB);
...
//        return false;
    ...
    qDebug() << "shadow_count_" << shadow_count_;

    qDebug() << "calib_range" << fabsf(calib_scan_response_ - expected_encoder_delta)/expected_encoder_delta;
    // scan backwards
    i = 0;
    axis_->run_control_loop([&](){
        //qDebug() << "3333";
...
bool Encoder::update() {
    ...
    int32_t interpolated_enc;
    float ph;

    if(axis_->motor_.config_.is_simulation == true)
    {
        elec_rad_per_enc = axis_->motor_.config_.pole_pairs * 2 * M_PI * (1.0f / (float)(config_.cpr));

        pos_estimate_ = axis_->motor_.virtual_motor.phi / elec_rad_per_enc;
        vel_estimate_ = axis_->motor_.virtual_motor.we;

        shadow_count_ = pos_estimate_;
        count_in_cpr_ = pos_estimate_;
        count_in_cpr_ = mod(count_in_cpr_, config_.cpr);

        interpolated_enc = count_in_cpr_ - config_.offset;
    ...
    {
        int32_t delta_enc = 0;
        tim_cnt_sample_ = axis_->motor_.virtual_motor.phi / 2.0 / M_PI * config_.cpr / axis_->motor_.config_.pole_pairs;
        switch (config_.mode) {
        case MODE_INCREMENTAL: {
            //TODO: use count_in_cpr_ instead as shadow_count_ can overflow
            //or use 64 bit
            int32_t delta_enc_16 = (int32_t)tim_cnt_sample_ - (int32_t)shadow_count_;
        ...
        interpolated_enc = corrected_enc + interpolation_;
